...
def mha(x, attn, past_key_value, n_head):  # [n_seq, n_embd] -> [n_seq, n_embd]
    ...
    if len(past_key_value) == 0:
        ...
        q = linear(x, q_wb)
        past_key_value[0] = torch.cat((past_key_value[0], linear(x, k_wb)), dim=-2)
        past_key_value[1] = torch.cat((past_key_value[1], linear(x, v_wb)), dim=-2)
...
def generate(inputs, params, n_head, n_tokens_to_generate):
    ...
    for _ in tqdm(range(n_tokens_to_generate), "generating"):  # auto-regressive decode loop
        logits = gpt2(inputs if next_id is None else next_id, params, past_key_values, n_head=n_head)  # model forward pass
